...
Script Purpose: 
    ...
    - Truncates existing tables bebfore loading data
    - Uses 'BULK INSERT' command to load the data from csv files to the tables

    Parameters:
        None. This stored procedure does not accept any Parameters nor return any values.
...
CREATE OR ALTER PROCEDURE bronze.load_bronze AS -- creating a stored procedure for loading data in bronze layer
BEGIN
    ...
    BEGIN TRY
        ...
        WITH (
            FIRSTROW = 2, -- as the dataset contains headers/column names in first row, the actual data starts from the second.
            FIELDTERMINATOR = ',', -- the separator or delimiter that the csv file uses.
            TABLOCK
        );
